            ...
            builder: (ctx, details, child) {
              ...
              return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Row(children: [
                  ...
                  QuantityButton(isIncrement: false, quantity: details.quantity, stock: _stock, minimumOrderQuantity: int.parse("${widget.product.minimumOrderQuantity}"),),
                  Text(details.quantity.toString(), style: titilliumSemiBold),
                  QuantityButton(isIncrement: true, quantity: details.quantity, stock: _stock,minimumOrderQuantity: int.parse("${widget.product.minimumOrderQuantity}")),
                ]),
                SizedBox(height: Dimensions.PADDING_SIZE_SMALL),
                ...
                Row(children: [
                  ...
                  Expanded(
                    child: CustomButton(buttonText: getTranslated(_stock < int.parse("${widget.product.minimumOrderQuantity}") ?
                    'out_of_stock' : 'add_to_cart', context),
                        onTap: _stock < int.parse("${widget.product.minimumOrderQuantity}")  ? null :() {
                          print("564298466495");
                          print(_stock);
                          print("${widget.product.minimumOrderQuantity}");
                          if(_stock >= int.parse("${widget.product.minimumOrderQuantity}") ) {
                            CartModel cart = CartModel(
                                ...
                                int.parse("${widget.product.minimumOrderQuantity}")
                            );
...
print("564298466495");
                  ...
                  Expanded(
                    child: CustomButton(isBuy:true,
                        buttonText: getTranslated(_stock <   int.parse("${widget.product.minimumOrderQuantity}") ? 'out_of_stock' : 'buy_now', context),
                        onTap: _stock <  int.parse("${widget.product.minimumOrderQuantity }") ? null :() {
                          if(_stock > int.parse("${widget.product.minimumOrderQuantity}") ) {
                            CartModel cart = CartModel(
                                widget.product.id, widget.product.thumbnail, widget.product.name,
