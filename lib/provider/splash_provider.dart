...
class SplashProvider extends ChangeNotifier {
  ...
  Future<bool> initConfig(BuildContext context) async {
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      ...
      print("_currencyCode$_currencyCode");
      print("_configModel.currencyList${_configModel.currencyList.length.toString()}");

      for(CurrencyList currencyList in _configModel.currencyList) {
        if(currencyList.id == _configModel.systemDefaultCurrency) {
          print("_configModel.systemDefaultCurrency${_configModel.systemDefaultCurrency}");

          print("currencyList.id${currencyList.id}");
          if(_currencyCode == null || _currencyCode.isEmpty) {
            _currencyCode = currencyList.code;
            print("currencyList.code${currencyList.code.toString()}");
          }
      ...
      print("_currencyCodeas$_currencyCode");
  ...
  void getCurrencyData(String currencyCode) {
    _configModel.currencyList.forEach((currency) {
      if(currencyCode == currency.code) {
        ...
        _myCurrency = currency;  print("currerntc$_myCurrency");
  ...
  void setCurrency(int index) {
    ...
    getCurrencyData(_configModel.currencyList[index].code);  print("asdssssssad${_configModel.currencyList[index].code}");
    notifyListeners();
  }
